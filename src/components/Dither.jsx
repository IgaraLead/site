...
const RetroEffect = forwardRef(function RetroEffect(props, ref) {
...
function DitheredWaves({
  ...
  // Material criado imperativamente UMA vez e anexado por referência:
  // passar `uniforms` via JSX faz o R3F/three clonar o objeto, e as
  // mutações por frame (ex. `time`) nunca chegariam à GPU (tela congelada).
  ...
  const prevBackgroundColor = useRef(null);
  useFrame(({ clock }) => {
    ...
    if (!prevColor.current || !prevColor.current.every((v, i) => v === waveColor[i])) {
      u.waveColor.value.set(...waveColor);
      prevColor.current = [...waveColor];
    }

    if (
      !prevBackgroundColor.current ||
  ...
  // Rastreia o mouse na janela inteira (e não só sobre o canvas):
  // os textos/botões do hero ficam acima do canvas e interceptam os
  // eventos, o que congelava o efeito interativo sobre eles.
  useEffect(() => {
...
export default function Dither({
  ...
  return (
    <div className={`h-full w-full overflow-hidden ${className}`.trim()} style={style}>
      <Canvas
